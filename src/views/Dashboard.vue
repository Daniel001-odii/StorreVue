        ...
        gap: 15px;">
            ...
            <!-- <div class="inner" style="border: 1px solid red;"> -->
                ...
                ">
                    ...
                    gap: 10px;
                    margin: 20px 0px;">
                    <StatBox v-if="user.customers">
                        <template #icon><i class="bi bi-people"></i></template>
                            <template #number>{{ user.customers.length }}</template>
                            <template #item>New Customers</template>
                    </StatBox>
                    <StatBox>
                            <template #icon><i class="bi bi-box"></i></template>
                            <template #number>{{total_income.allProducts}}</template>
                            <template #item>Products Available</template>
                    </StatBox>
                    <StatBox v-if="user.stores">
                            <template #icon><i class="bi bi-shop"></i></template>
                            <template #number>{{ user.stores.length }}</template>
                            <template #item>Stores Owned</template>
                    </StatBox>
                    </div>
                    <div style="color: #6147DB; font-weight: 700;"><i class="bi bi-graph-up"  style="font-size: 16px; margin-right: 5px;"></i>See all analytics</div>
                    ...
                    flex-direction: row;">
                        ...
                        border-radius: 10px;
                        background: #6147DB;
                        box-shadow: 0px 4px 50px 0px #6147db59;
                        display: flex;
                        gap: 8px;
                        border: none;
                        padding: 5px 30px;
                        align-items: center;
                        color: #fff;" @click="activityModal = !activityModal"><i class="bi bi-tag-fill"></i>New Order</button>
                    </div>
                    <!-- Recent activity and new order button ends here -->

                    <!-- activity records details starts here -->
                    <div class="activity_scroll" style="">
                    <div class="activity_date" style="font-weight: 700; border-bottom: 1px solid #D2D2D2; padding-bottom: 20px;" v-if="userActivity">TODAY | {{ formatTimestamp(userActivity[0].createdAt) }}</div>

                    <!-- activity line starts here---------------- -->
                    <div class="activity_line"  v-for="(activity, activityIndex) in userActivity.slice(0, 5)" :key="activityIndex" style="">
                        <div class="activityLine_first" style="">
                            <div class="activity_in" style="">
                                <i class="bi bi-plus" style="color: #12BC95;"  v-if="activity.type == 'income'"></i>
                                <i class="bi bi-dash" style="color: red;" v-if="activity.type == 'expense'"></i>
                            </div>
                            <!-- {{ activity }} -->
                            <div class="activity_text" style="display: flex; flex-direction: column;">
                                <span>Sold <b>{{activity.product.name}}</b></span>
                                <span>to {{ activity.customer.name }}</span>
                            </div>
                            
                        </div>
                        <div class="activityLine_second">${{activity.amount}}</div>
                    </div> 
                    <!-- activity line ends here---------------- -->

                    <div class="see_more" style="width: 100%; text-align: center;">see more</div>
                </div>
                </div>
            <!-- </div> -->
                
                <div class="inner_second" style="
                width: 300px;
                height: 600px;
                background: #fff;
                border-radius: 20px;">
